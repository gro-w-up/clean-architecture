...
- **시스템 컴포넌트인 테스트**
    - 테스트의 종류가 무엇이건, 테스트는 모두 아키텍처적으로 동등하다.
    - 너무 당연한 말이지만 테스트는 세부적이며, 시스템의 컴포넌트에 의존한다.
    - 테스트는 독립적으로 배포 가능하다. 보통 상용 시스템과 별개로 독립적으로 배포한다.

- **테스트를 고려한 설계**
    - 테스트가 시스템의 설계와 잘 통합되지 않으면 테스트는 깨지기 쉬워지고, 시스템은 뻣뻣해져서 변경하기가 어려워진다.
    - 테스트는 시스템에 극도로 의존적이기 때문에, 시스템의 사소한 변화로도 테스트는 깨지기 쉽고, 그 때문에 시스템에 변동을 주기 꺼려하게 된다. 
    - 시스템과 테스트를 설계할 때, GUI처럼 변동성이 큰 것을 갖고 설계하면 안된다.

- **테스트 API**
    - 위와 같은 목표를 달성하기 위해, 테스트가 모든 업무 규칙을 검증하는 데 사용할 수 있도록 특화된 API를 만들면 된단다,,
    - 테스트 API는 테스트 구조를 애플리케이션 구조로부터 결합을 분리하는 게 목표다.
    - 테스트 API는 애플리케이션의 구조를 테스트로부터 숨기는데, 이렇게 만들면 상용 코드를 리팩토링하거나 진화시키더라도 테스트에는 전혀 영향을 주지 않는다고 한다. 테스트 API는 도대체 뭘까..?
    - 이 같은 것들이 중요한 이유는, 테스트코드와 상용코드는 진화의 방향이 정반대이기 때문이다. 테스트는 구체화 되고 상용코드는 추상화 되기 때문이다. 구조적 결합이 강하면 안된다.

- **결론**
    - 테스트를 시스템의 일부로서 취급하고 설계를 잘 해야 테스트를 유지보수할 수 있다.
    - (근데 테스트 API 라는건 뭐길래 결합성을 줄일 수가 있는거지..?)

<br>

## 29장. 클린 임베디드 아키텍처

- 소프트웨어의 아키텍처를 잘 구성해야 한다고 설명하면서, 그 이유로 펌웨어를 가져왔다.
- 코드에 하드웨어에 대한 의존성을 퍼뜨려놓는다면 펌웨어랑 다를바가 없다고 한다. 이 말에서 펌웨어의 정의를 알아차렸는데, 하드웨어에 의존적인 소프트웨어를 펌웨어라고 하는거다.
- 소프트웨어가 내장된 기기들 말이다, 그 기기 안에 있는 소프트웨어가 펌웨어인것이다.
- 그니까, 앱 같은 서비스를 만들었는데, 그게 ios에 의존적이어서 ios 업데이트마다 같이 업데이트를 해야한다면 그게 펌웨어랑 다를 바가 무엇이냐. 이 말이다.
- 하드웨어는 빠르게 발전하고 진화한다. 이전의 하드웨어는 폐기되겠지만 소프트웨어는 기존의 것을 계속 개선시켜나가며 사용할 수 있다. 하지만 그러기 위해서는 계속해서 바뀌어가는 하드웨어와 명확히 분리되어야만 해당 소프트웨어를 오랫동안 유용하게 사용할 수 있다.
...
- **앱-티튜드 테스트**
    ...
    - 임베디드 소프트웨어 엔지니어들이 첫번째 혹은 세번째 활동에만 중점을 두고 작성했기 때문에 수많은 임베디드 시스템이 펌웨어로 변하는 것 같다고 한다.
    - 저자는 앱이 동작하도록 만드는 것을 개발자용 앱-티튜드 테스트(App-ttitude test)라고 부른단다.
    - 그리고 앱-티튜드 테스트만 통과한다고(동작만 원활히 된다고) 능사가 아니다. 클린 임베디드 아키텍처가 필요하다.
- **타깃-하드웨어 병목현상**
    - 임베디드가 지닌 특수한 문제 중 하나는 타깃-하드웨어 병목현상(target-hardware bottleneck)이다.
    - 임베디드 개발은 하드웨어, 소프트웨어, 펌웨어가 거의 동시에 만들어지고, 해당 소프트웨어 개발자는 개발 시점에 코드를 실행할 대상조차 없을 수 있다.
    - 임베디드 코드가 클린 아키텍처 원칙과 실천법을 따르지 않은채 작성된다면, 해당 코드를 테스트 할 수 있는 환경은 특정 타깃(특정 하드웨어)로 국한될 것이고, 그 타깃이 테스트 할 수 있는 유일한 장소라면 타깃-하드웨어 병목현상이 발생하여 진척이 느려질 것이다.
    - **클린 임베디드 아키텍처는 테스트하기 쉬운 임베디는 아키텍처다.**
        - **계층**
            - 우선 계층은 크게 세가지, ‘소프트웨어’ > ‘ 펌웨어’ > ‘하드웨어’ 로 나뉜다.
            - 최소한 하드웨어 & 나머지 는 분명하게 나뉘어야한다. 그러지 않고서는 하드웨어의 변경 혹은 소프트웨어의 사소한 변경까지도 상당한 영향을 미치게 될 것이다.
            - 소프트웨어와 펌웨어가 서로 섞이는 일은 안티 패턴(anti-pattern)이다. 안티 패턴을 보이는 코드는 변화에 저항하게 된다.
        - **하드웨어는 세부사항이다.**
            - 하드웨어&코드 그 사이의 경계보다도, 코드에서 펌웨어&소프트웨어 사이의 경계를 정의하기는 힘들다.
            - 임베디드 소프트웨어 개발자가 해야 할 일 하나가 바로 이 경계를 분명하게 만드는 것이다.
            ...
            - 예를 들어, 소프트웨어는 본인이 사용하는 데이터 형태가 하드웨어에 저장되면 되지, 그게 어디 저장되건 어떤식으로 저장되건 알 바 아닌것이다. 그것을 위해 존재하는게 HAL이다.
    - **HAL 사용자에게 하드웨어 세부사항을 드러내지 말라**
        - **프로세서는 세부사항이다.**
            - 임베디드 소프트웨어가 프로세서에 종속적이면 안정적 하드웨어 출시 이전에, 그리고 다른 프로세서로 이식할 때 문제가 될 수 있다.
            - 세부 내용은 온전히 이해하지는 못했다,,
        - **운영체제는 세부사항이다.**
            - HAL은 필수적이다. 하지만 HAL만으로 충분하지 않다.
            - 운영체제를 세부사항으로 취급해 의존하지 말아야한다.
            - 소프트웨어는 운영체제를 통해 운영 환경이 제공하는 서비스에 접근한다. OS는 소프트웨어를 펌웨어로부터 분리하는 계층이다.
                - 소프트웨어 > OS > 펌웨어 > 하드웨어
            - 소프트웨어가 OS 에 의존하게 되면, 갖가지 변수들로 인해 해당 OS 를 사용하지 못하게 됐을 때 상당한 변경을 필요로 한다.
            - OS와 소프트웨어 사이의 격리층은 OSAL(Operating System Abstraction Layer)이라고 한다.
                - 소프트웨어 > ‘OSAL’ > OS > ‘HAL’ > 펌웨어 > 하드웨어
            - OSAL이 존재하면 OS 를 변경하게 됐을 때, OS와 엮인 소프트웨어를 직접 수정하는 대신, 기존의 OSAL과 호환되도록 새로운 OSAL을 작성하면 된다.
    - **인터페이스를 통하고 대체 가능성을 높이는 방향으로 프로그래밍하라**
        - 관심사를 분리시키고, 인터페이스를 활용하며, 대체 가능성을 높이는 방향으로 프로그래밍하라.
        - 모듈들이 인터페이스를 통해 상호작용한다면 특정 서비스 제공자를 다른 제공자로 대체할 수 있다.
    - **DRY 원칙: 조건부 컴파일 지시자를 반복하지 말라**
        - Don’t Repeat Yourself
        - 임베디드 시스템의 경우 타깃-하드웨어의 유형을 식별하는 조건부 컴파일을 반복해서 사용할 때가 많다.
            - #ifdef BOARD_V2
        - 대안으로 하드웨어 추상화 계층은 어떨까? 라고 하는데,, 펌웨어와 하드웨어 사이에 추상화 계층을 놓겠다는 것일까,, 이 계층에 조건부 컴파일 대신 사용할 수 있는 일련의 인터페이스를 제공하면 된다고 한다.
- **결론**
    - 임베디드 소프트웨어가 펌웨어가 되도록 하지 말아라. (하드웨어 의존적이게 하지 말아라)
